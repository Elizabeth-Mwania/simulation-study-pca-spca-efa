# PCA,SPCA WITH FIXED ALPHA=0.5, 4 EFA VARIANTS 
...
output_dir <- "simulation_final_result1"
...
apply_sparse_pca_fixed <- function(X, m, fixed_alpha = 0.5) {
  n <- nrow(X)
  p <- ncol(X)
...
run_simulation <- function(params) {
  ...
  tryCatch({
    ...
    rep <- params$rep
    fixed_alpha <- 0.5  # Using fixed alpha = 0.5 for Sparse PCA
    ...
    create_result_row <- function(method_name, eval_res, used_alpha = NA, rot_method = NA) {
      data.frame(
        method = method_name,
        mse = eval_res$mse,
    ...
    methods <- c("PCA", "Sparse PCA (Fixed)", "EFA (varimax)", "EFA (oblimin)", 
                "EFA (quartimin)", "EFA (quartimax)")
    rotation_methods <- c(NA, NA, "varimax", "oblimin", "quartimin", "quartimax")
...
params <- expand.grid(
  ...
  gamma = c(0, 1, 2),
  sigma_sq = c(0.1, 1, 5),
  rep = 1:1  # 1 repetition per scenario (change to more if needed)
)
...
results_list <- foreach(i = 1:nrow(params), .packages = c("psych", "sparsepca", "GPArotation", "dplyr", "vegan"),
...
aggregated_results <- results %>%
  group_by(method, n, p, m, alpha, gamma, sigma_sq, rotation_method) %>%
  summarize(
